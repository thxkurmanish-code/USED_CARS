...
interface Listing { id: string; brand: string; model: string; manufacturing_year: number; status: string; price: string; description: string; }
export default function ListingManagePage() {
  const params = useParams<{ id: string }>(); const router = useRouter(); const { user, loading } = useAuth(); const [listing, setListing] = useState<Listing | null>(null); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false);
  useEffect(() => { if (user) apiClient<Listing[]>("/listings/mine").then((items) => setListing(items.find((item) => item.id === params.id) ?? null)).catch(() => setError("Listing could not be loaded.")); }, [params.id, user]);
